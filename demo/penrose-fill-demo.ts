...
function highlightTriNeighbors(selector: string, coord: string): void {
  const neighbors = [0, 1, 2].map((side) =>
    tatham_neighbor_or_null(coord, side as 0 | 1 | 2)
  );
...
function drawTriangles(
  ...
  d3.select(`${selector} g#triangles`)
    ...
    .attr(
      'd',
      (tri: Triangle) =>
        `M ${xform(tri.v1.x)}, ${yform(tri.v1.y)} L ${xform(tri.v2.x)}, ${yform(tri.v2.y)} L ${xform(tri.v3.x)}, ${yform(tri.v3.y)} Z`
    )
    .style('fill', (tri: Triangle) => tri.fillColor)
    .on('mouseover', (_: Event, d: Triangle) =>
      highlightTriNeighbors(selector, d.coord)
    );
  if (showIndex) {
    const maxCoordLen = 9;
    d3.select(`${selector} g#triangles`)
      .selectAll('text.robinson')
      .data(allTris)
      .join('text')
      .attr('class', 'robinson')
      .attr('x', (tri: Triangle) => xform((tri.v1.x + tri.v2.x + tri.v3.x) / 3))
      .attr('y', (tri: Triangle) => yform((tri.v1.y + tri.v2.y + tri.v3.y) / 3))
      .style('opacity', ({ coord }: Triangle) =>
        d3.easeCubicOut((maxCoordLen - coord.length) / maxCoordLen)
      )
  ...
  d3.select(`${selector} g#polygon`)
    ...
    .attr(
      'd',
      () =>
        `M ${polygon[0].print(xform, yform)} ` +
        polygon
          .slice(1)
          .map((v) => v.print(xform, yform))
          .join(' ') +
        ' Z'
    );
}

function highlightRhombNeighbors(
  selector: string,
  rhombhash: RhombusMap,
  coord: string | null
): void {
  const neighbors = coord
    ? (rhombhash[coord]?.neighbors ?? [null, null, null, null])
    : [null, null, null, null];
...
function drawRhombuses(
  ...
  const categorical_colors = (d3.schemeTableau10 ?? [])
    ...
    .filter(Boolean);
  if (!categorical_colors.length) categorical_colors.push('#888');
  d3.select(`${selector} g#rhombuses`)
    ...
    .attr(
      'd',
      ({ rhombus: rh }: RhombusEntry) =>
        `M ${xform(rh.v1.x)}, ${yform(rh.v1.y)} L ${xform(rh.v2.x)}, ${yform(rh.v2.y)} L ${xform(rh.v3.x)}, ${yform(rh.v3.y)} L ${xform(rh.v4.x)}, ${yform(rh.v4.y)} Z`
    )
    .style('fill', (entry: RhombusEntry) =>
      showBaseRhombuses && entry.base !== null
        ? categorical_colors[entry.base % categorical_colors.length]
        : entry.rhombus.fillColor
    )
    .on('mouseover', (_: Event, d: RhombusEntry) =>
      highlightRhombNeighbors(selector, rhombhash, d.rhombus.coord)
    )
  ...
  if (showBaseRhombuses) {
    d3.select(`${selector} g#rhombuses`)
      ...
      .join('text')
      .attr('class', 'robinson')
      .attr('x', ({ rhombus }: RhombusEntry) =>
        xform((rhombus.v1.x + rhombus.v2.x + rhombus.v3.x + rhombus.v4.x) / 4)
      )
      .attr('y', ({ rhombus }: RhombusEntry) =>
        yform((rhombus.v1.y + rhombus.v2.y + rhombus.v3.y + rhombus.v4.y) / 4)
      )
      .text((entry: RhombusEntry) => entry.base ?? '');
  }
  if (showIndex || showSides) {
    const triangles: TriangleWithCenter[] = rhombEntries.flatMap(
      ({ tri1scale, tri2scale }) => {
    ...
    if (showIndex) {
      d3.select(`${selector} g#coord`)
        ...
        .attr(
          'class',
          ({ tri }: TriangleWithCenter) => 'robinson ' + tri.coord.slice(0, 1)
        )
    ...
    if (showSides === 'tri') {
      const sides: TriangleSideLabel[] = triangles.flatMap(
        ({ tri, center }) => {
          return [0, 1, 2]
            .map((i) => {
      ...
      d3.select(`${selector} g#coord`)
        ...
        .attr(
          'd',
          ({ v2, v4 }: Rhombus) =>
            `M${xform(v4.x)},${yform(v4.y)} L${xform(v2.x)},${yform(v2.y)}`
        )
        .style('stroke', 'black')
      ...
      d3.select(`${selector} g#coord`)
        ...
        .attr(
          'class',
          ({ coord }: TriangleSideLabel) => 'side ' + coord.slice(0, 1)
        )
...
function getInputValues(): InputValues {
  ...
  const boundsShapeElement = document.querySelector<HTMLInputElement>(
    'input[name="init_shape"]:checked'
  );
  const boundsShape = (boundsShapeElement?.value ?? 'square') as BoundsShape;
  const resolveElement = document.querySelector<HTMLInputElement>(
    'input[name="resolve_ragged"]:checked'
  );
...
function drawPenroseTiling(): void {
  ...
  if (!gnomon || !tiles) throw new Error('Required SVG elements not found');
  ...
  const {
    ...
    inputs.boundsShape,
    startile,
    inputs.resolveRagged,
    fixedCenter,
    fixedR
  );
  const dt = performance.now() - startt;
  const degreeCounts = Object.values(p3Rhombuses).reduce(
    (counts, { neighbors }) => {
  ...
  const totalBorder = degreeCounts
    .slice(0, 4)
  ...
  const degreeText = degreeCounts
    ...
    .map((count, degree) =>
      count
        ? `<div>border degree ${degree}: ${(totalBorder ? (count / totalBorder) * 100 : 0).toFixed(0)}%</div>`
        : ''
    )
    .join('');
  const fixLink =
...
<div>rhombs culled: ${culledRhombuses.length}</div>`
        : inputs.resolveRagged === 'fill'
          ...
          : '') +
      degreeText +
      (showBaseRhombuses ? `<div>rhombus bases: ${rhombBases.length}` : '') +
  ...
  drawTriangles(
    'svg#gnomon',
    robinsonTriangles,
    discardedTriangles.concat(culledTriangles),
    polygon
  );
  ...
  const { tl, br } =
    drawlevel === 'triangle'
      ...
      : calculateRhombusesBB(
          Object.values(p3Rhombuses).map(({ rhombus }) => rhombus)
        );
...
const startile: TriangleKind =
  startParam && allowedStartTiles.includes(startParam as TriangleKind)
    ? (startParam as TriangleKind)
    : 'X';
const coordParam = urlParams.get('coord');
let showIndex: number | null = null;
if (coordParam !== null) {
  if (['true', ''].includes(coordParam.toLowerCase())) showIndex = 2;
...
const drawlevel: 'triangle' | 'rhombus' =
  urlParams.get('draw') === 'triangle' ? 'triangle' : 'rhombus';
...
const shapeInputs = Array.from(
  document.querySelectorAll<HTMLInputElement>('input[name="init_shape"]')
);
if (shape !== null && shapeInputs.some((elem) => elem.value === shape)) {
  shapeInputs.forEach((elem) => {
    elem.checked = elem.value === shape;
  });
}
const raggedInputs = Array.from(
  document.querySelectorAll<HTMLInputElement>('input[name="resolve_ragged"]')
);
if (ragged !== null && raggedInputs.some((elem) => elem.value === ragged)) {
  raggedInputs.forEach((elem) => {
    elem.checked = elem.value === ragged;
  });
}
